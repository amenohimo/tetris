...
**目標**: スマホやタブレットのタッチパネルで遊べるようにします。スワイプ・フリック・タップによる直感的な操作を実現します。

**学べること**: タッチイベント（`touchstart`/`touchend`）、スワイプとフリックの判定、マルチタッチ、モバイルブラウザの制御

#### 操作設計

キーボードと違い、タッチには「押しっぱなし」の概念がありません。以下のような操作体系を最初に決める必要があります。
...
**タップ / スワイプ / フリックの違い:**
- **タップ**: 指を置いてすぐ離します。移動距離が小さい場合です。クリックに相当します
- **スワイプ**: 指をなぞって離します。**移動距離**で判定します
- **フリック**: 指を勢いよくはじいて離します。**移動速度**で判定します

テトリスでは「下になぞる → ソフトドロップ」「下にはじく → ハードドロップ」と割り当てると直感的です。

#### 実装上の注意点

| 問題 | 対策 |
|------|------|
| **DAS/ARR がタッチに使えない** | キーボードの「押しっぱなし連続移動」はタッチに存在しません。タップ連打またはタッチ用の独自リピート処理を実装します |
| **ブラウザのデフォルト動作が邪魔** | `touch-action: none` をCSSに設定し、`e.preventDefault()` でスクロール・ピンチズーム・プルリフレッシュを全て潰します |
| **マルチタッチの競合** | 2本指で同時操作するケース（左移動しながら回転など）を考慮します。`touches` 配列で複数指を管理します |
| **スワイプとフリックの誤判定** | 「ゆっくり降ろす」と「一気に落とす」の境界が曖昧です。`touchend` 時の移動速度（`距離 / 時間`）で閾値を設定し、実機で調整します |
| **タッチ領域が狭い** | 盤面の左右余白もタップ領域にします。盤面上のタップは回転、盤面外の左右は移動、と領域を分けます |
| **触覚フィードバックがない** | 押したつもりが押せてない事故が起きやすいです。視覚的フィードバック（タップ時に一瞬光るなど）を入れると改善します |
| **テストが困難** | エミュレータではタッチ速度や同時押しが再現できません。実機で確認します |
