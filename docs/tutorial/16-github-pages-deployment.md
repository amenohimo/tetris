...
とはいえ、「サーバーを借りる」「ドメインを取得する」といった作業は敷居が高いですよね。そこで登場するのが **GitHub Pages** です。GitHub Pages を使えば、それらの面倒な作業が**一切不要**で、Web サイトを無料で公開できます。
...
| **ビルド不要のホスティング** | このテトリスは Vite でビルド済みの静的ファイル（HTML + CSS + JS）なので、サーバーサイドの処理は一切不要。Pages に最適 |
...
これはなぜそうなっているのでしょうか。GitHub Pages の設計思想として、**ソースコードの管理** と **Web サイトの公開** は別物だからです。
...
よくある誤解が **「GitHub Pages が main ブランチを公開している」** というものです。実際には Pages は `gh-pages` ブランチだけを見ています。この分離により、以下のメリットがあります。

- **ソースコードを公開したくない場合**でも、ビルド成果物だけを公開できる（Privateリポジトリ + 有料プランでの利用）
- **公開用ファイルの履歴を残さない** → リポジトリが肥大化しない
- **ビルド結果とソースが混ざらない** → 何を管理すべきかが明確
...
**注意:** 初回だけは方法Aと同じく GitHub Pages の設定を有効にする必要があります。方法Aで一度デプロイしてから切り替えるとスムーズです。
